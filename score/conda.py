...
for package_name in list(packages_name_list.keys())[:1]:
    ...
    if soup:
        license = str(soup.select_one('[title="License"]').text).split(":")[-1].strip() if soup.select_one('[title="License"]') else None
        print(license)
        
        homepage_element = soup.select_one('[title="Home Page"]')
        homepage_url = homepage_element.attrs['href'] if homepage_element and 'href' in homepage_element.attrs else None
        print(homepage_url)
        
        development_element = soup.select_one('[title="Development Url"]').select_one("a")
        development_url = development_element.attrs['href'] if development_element and 'href' in development_element.attrs else None
        print(development_url)
        
        documentation_element = soup.select_one('[title="Documentation Url"]').select_one("a")
        documentation_url = documentation_element.attrs['href'] if documentation_element and 'href' in documentation_element.attrs else None
        print(documentation_url)
        
        download_count_element = soup.select_one('[title="Download Count"]')
        download_count = int(str(download_count_element.select_one("span").text).strip()) if download_count_element.select_one("span") else None
        print(download_count)
        
        last_upload = str(soup.select_one('[title="Last upload"]').text).split(":")[-1].strip() if soup.select_one('[title="Last upload"]') else None
        print(last_upload)
    ...
    if soup:
        versions_list = [str(i.text).strip() for i in soup.select("#Version > li") if str(i.text).strip() != "All"]
